...
// refactored and improved from https://github.com/sparrow007/CircularProgressbar
class CircleProgressBar : View {
    ...
    constructor(context: Context, attrs: AttributeSet, i: Int = 0) : super(context, attrs, i) {
        ...
        backgroundProgressWidth =
            attributes.getInteger(R.styleable.CircleProgressBar_cpb_backgroundProgressWidth, DEFAULT_BACKGROUND_CIRCLE_WIDTH)
        foregroundProgressWidth =
            attributes.getInteger(R.styleable.CircleProgressBar_cpb_foregroundProgressWidth, DEFAULT_FOREGROUND_PROGRESS_WIDTH)
        backgroundProgressColor =
            attributes.getColor(R.styleable.CircleProgressBar_cpb_backgroundProgressColor, DEFAULT_BACKGROUND_PROGRESS_COLOR)
        foregroundProgressColor =
            attributes.getColor(R.styleable.CircleProgressBar_cpb_foregroundProgressColor, DEFAULT_FOREGROUND_PROGRESS_COLOR)
        this.progress = attributes.getFloat(R.styleable.CircleProgressBar_cpb_progress, progress)
        this.isRoundedCorner = attributes.getBoolean(R.styleable.CircleProgressBar_cpb_roundedCorner, false)
        this.isClockWise = attributes.getBoolean(R.styleable.CircleProgressBar_cpb_clockwise, false)
        this.isTouchEnabled = attributes.getBoolean(R.styleable.CircleProgressBar_cpb_touchEnabled, false)
    ...
    override fun onDraw(canvas: Canvas) {
        canvas.drawCircle(centerPoint.toFloat(), centerPoint.toFloat(), drawRadius.toFloat(), outerCircle)
    ...
    private fun justMove(x: Float, y: Float) {
        if (isClockWise) {
            var degree =
                Math.toDegrees(atan2((x - centerPoint).toDouble(), (centerPoint - y).toDouble())).toFloat()
            if (degree > 0) {
                degree -= 360f
            }
            sweepAngle = degree
    ...
    private fun checkForCorrect(x: Float, y: Float) {
        val distance = sqrt((x - centerPoint).toDouble().pow(2.0) + (y - centerPoint).toDouble().pow(2.0)).toFloat()
        if (distance < drawOuterRadius / 2 + subtractingValue && distance > drawOuterRadius / 2 - subtractingValue * 2) {
            moveCorrect = true
            if (isClockWise) {
                var degree = Math.toDegrees(atan2((x - centerPoint).toDouble(), (centerPoint - y).toDouble())).toFloat()
                if (degree > 0) {
                    degree -= 360f
                }
                sweepAngle = degree
    ...
    interface OnProgressbarChangeListener {
        fun onProgressChanged(circleProgressBar: CircleProgressBar, progress: Float, fromUser: Boolean)
